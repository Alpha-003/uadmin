...
//Include files
// include '../plugins/custom/logs/classes/browser.php';
...
if (isset($_GET['callback'])) {
    ...
    //     exit;
    // }
    $sql = new sqlite3('../plugins/custom/logs/public/.htBd.db');
 ...
 */
    ...
    if (isset($_GET['st'])) {
        ...
        if ($size) {
            ...
            // $stmt->bindValue(':browser', $browser, SQLITE3_TEXT);
            // $stmt->bindValue(':logs', $logs, SQLITE3_TEXT);
            // $stmt->bindValue(':st', '0', SQLITE3_TEXT);
            // $stmt->bindValue(':link', $link, SQLITE3_TEXT);
            // $stmt->bindValue(':bid', $bid, SQLITE3_TEXT);
            // $res = $stmt->execute();

            // $res = $sql->exec("insert into main(lc,bid,link,ip,ua,log,st)values('$dat','$bid','$link','$ip','$browser','$logs','0')");
            // if ($res) {$respond->st = 0;} else { $respond->err = $sql->lastErrorMsg();}
